...
#include <ws2tcpip.h>
    ...
    // Define InetPtonA if not defined
    #ifndef InetPtonA
int InetPtonA(int af, const char *src, void *dst) {
    struct sockaddr_in sa;
    int size = sizeof(sa);
    char addressString[INET_ADDRSTRLEN];
    strncpy(addressString, src, INET_ADDRSTRLEN);
    addressString[INET_ADDRSTRLEN - 1] = '\0';  // Ensure null-termination
    int result = WSAStringToAddressA(addressString, af, NULL, (struct sockaddr *)&sa, &size);
    if (result == 0) {
